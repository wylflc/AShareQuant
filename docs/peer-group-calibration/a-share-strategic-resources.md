...
## 1.5 Proposed Calibration Rules
...
4. **Strategic Small Metals Can Be Retained Despite Weak Current Profit**: `云南锗业` should not be rejected merely because current profit and cash flow are weak if the resource, material chain, and end-market evidence hold.
...
7. **Reclassify Mixed Material Platforms**: tungsten tools, battery materials, lithium converters, and nickel-cobalt precursor companies may be high quality, but should be compared in narrower groups before final watchlist decisions.
8. **Pure Gold Needs A Separate Thesis**: a gold company should be retained only if its mine assets, exploration, cost curve, or deep-mining capability are worth observing independently of gold price.

## 1.6 Analyst Dissent Policy
...
3. I would softly dissent if `云南锗业` is rejected only because current net profit or cash flow is weak. Rejection based on small scale and unproven execution is defensible.
4. I would not defend `中国铝业` as a top-five A-share quality company under the current evidence. It can be boundary watch, but the baseline rank was too high.
5. I would not dissent if `江西铜业`, `云南铜业`, or `中金黄金` remain excluded under the current evidence.

## 1.7 Remaining Work

1. Ask the reviewer to decide whether to retain boundary resource companies: `中国铝业`, `云南锗业`, `盐湖股份`, `山东黄金`, `厦门钨业`, and `中钨高新`.
2. Open separate peer groups for lithium resources, battery materials, tungsten materials and hard-alloy tools, and pure gold miners.
...
- 山东黄金 2025 result and resource coverage: `https://jjdb.sdenews.com/jjdb/jjdb/content/20260327/Articel01002MT.htm`, `https://www.cfi.net.cn/p20260326004883.html`
